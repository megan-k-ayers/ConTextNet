...
#' @param tune_method How tuning should be performed: locally, via a cluster job
#'        array, or not at all
...
prep_data <- function(x, y_name, text_name,  model_params, task,
  ...
  ### Prep dSQ job array if running on the cluster.
  if (tune_method == "cluster") {
    prep_cluster_tune(nrow(grid), compute_params$job_iters,
                      paste0(file.path(folder_path, folder_name),
                             "/input_embeds.rds"),
                      compute_params$conda_env, compute_params$cpu_mem,
                      compute_params$runtime, compute_params$cpu_num,
